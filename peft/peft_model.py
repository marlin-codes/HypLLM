...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def from_pretrained(cls, model, model_id, **kwargs):
        ...
        config.lora_type = kwargs.get("lora_type", config.lora_type)
        # pdb.set_trace() 
...
class PeftModelForCausalLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        get_lm_head_input = kwargs.pop('get_lm_head_input', False)
        
        if not isinstance(self.peft_config, PromptLearningConfig):
            lm_head_input = None

            if get_lm_head_input:
                def lm_head_input_hook(module, input, output):
                    nonlocal lm_head_input
                    lm_head_input = input[0]
                    
                lm_head = self.base_model.lm_head
                handle = lm_head.register_forward_hook(lm_head_input_hook)
